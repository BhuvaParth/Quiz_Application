...
import { PythonQuiz } from "../constant";
...
export default function Python() {
  ...
  const [answers, setAnswers] = useState(
    Array(PythonQuiz.questions.length).fill(null)
  );
  ...
  const handleNextQuestion = () => {
    if (selectedOption !== null) {
      ...
      if (currentQuestionIndex < PythonQuiz.questions.length - 1) {
  ...
  const { question, options } = PythonQuiz.questions[currentQuestionIndex];

  if (isSubmitted) {
    const score = answers.reduce((score, answer, index) => {
      if (answer === PythonQuiz.questions[index].correctAnswer) {
        return score + 1;
      }
      return score;
    }, 0);
    ...
    return (
      <Result
        score={score}
        total={PythonQuiz.questions.length}
        answers={answers}
  ...
  return (
    ...
    >
      ...
      >
        {currentQuestionIndex < PythonQuiz.questions.length - 1
          ? "Next"
          : "Submit"}
      </button>
    </div>
